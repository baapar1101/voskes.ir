...
- Rewrote every internal `href`/`src`/`action`/`value` reference from
  `/en/...` to `/...` (and `https://voskes.nl/en/...` to
  `https://voskes.nl/...`) across the header/footer/cookie-banner partials,
...
While doing this restructuring, `voskes.nl/` was found to no longer
...
verified again via `find voskes.nl -type f -newer <marker>`.

Also found on disk: `d:/voskes/voskes.nl-standard-html/voskes.nl/` — a
...
before any of this happened) stood in for `voskes.nl/en/*.html` in
...
be treated as precious until/unless `voskes.nl/` is restored properly.
...
Superseded mid-migration: the source tree (`voskes.nl/`) was independently
...
against `voskes.nl/`, pushing the new Persian copy into ~200 pages'
...
just re-read whatever `voskes.nl/` currently contains.
...
Partway through this migration, `voskes.nl/` changed underneath it —
...
`voskes.nl/` gained a git repo (`bee726c "first commit"`, remote
...
`voskes.nl/`, then re-verified all four §6 gates from scratch. All pass
against the updated source. `voskes.nl -type f -newer` was used to
confirm this migration never wrote into `voskes.nl/` itself at any point.
...
Started with no git repository anywhere. `voskes.nl/` gained one mid-session
...
`voskes.nl -type f -newer <marker>`, confirming this migration never wrote
into `voskes.nl/`.
